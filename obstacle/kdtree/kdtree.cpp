...
class compare_dimension {
  public:
    ...
    bool operator()(const KdNode &p, const KdNode &q) const { return (p.point[d] < q.point[d]); }
    size_t d;
};
  ...
  public:
    ...
    double distance(const CoordPoint &p, const CoordPoint &q) {
        ...
        for (size_t i = 0; i < p.size(); i++) {
            dist += coordinate_distance(p[i], q[i], i);
        }
        return dist;
    }
    double coordinate_distance(double x, double y, size_t dim) {
...
void KdTree::set_distance(int distance_type, const DoubleVector *weights /*=NULL*/) {
    ...
    if (distance_type == 0) {
        ...
        distance = dynamic_cast<DistanceMeasure *>(new DistanceL0(weights));
    } else if (distance_type == 1) {
        // distance = (DistanceMeasure*)new DistanceL1(weights);
        distance = dynamic_cast<DistanceMeasure *>(new DistanceL1(weights));
    } else {
        //    distance = (DistanceMeasure*)new DistanceL2(weights);
        distance = dynamic_cast<DistanceMeasure *>(new DistanceL2(weights));
    }
}
